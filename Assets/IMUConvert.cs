...
public class IMUConvert : MonoBehaviour
{
    ...
    void ProcessCsv(string filePath)
    {
        ...
        quatBase = new Quaternion(float.Parse(values[8]), float.Parse(values[9]), float.Parse(values[10]), float.Parse(values[7]));
        ...
        for (int i = 3; i < lines.Length; i++)
        {
            ...
            // Save First Accel Data
            acceleration1 = new Vector3(float.Parse(values[1]) + 1.0f, float.Parse(values[2]), float.Parse(values[3]));
            Quaternion quat1 = new Quaternion(float.Parse(values[8]), float.Parse(values[9]), float.Parse(values[10]), float.Parse(values[7]));
